...
public class DropDownMenu extends LinearLayout {
    ...
    private void addTab(@NonNull List<String> tabTexts, int i) {
        final LinearLayout lltab = new LinearLayout(getContext());
        lltab.setLayoutParams(new LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1.0f));
        lltab.setOrientation(HORIZONTAL);
        lltab.setGravity(Gravity.CENTER);
        final TextView tvtab = new TextView(getContext());
        final ImageView ivtab = new ImageView(getContext());
        tvtab.setSingleLine();
        tvtab.setEllipsize(TextUtils.TruncateAt.END);
        tvtab.setGravity(Gravity.CENTER);
        tvtab.setTextSize(TypedValue.COMPLEX_UNIT_PX, menuTextSize);
//        tvtab.setLayoutParams(new LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1.0f));
        tvtab.setTextColor(textUnselectedColor);
        ivtab.setImageResource(menuUnselectedIcon);
        tvtab.setText(tabTexts.get(i));
        tvtab.setPadding(dpTpPx(5), dpTpPx(12), dpTpPx(5), dpTpPx(12));
        //添加点击事件
    ...
    public void closeMenu() {
        if (current_tab_position != -1) {
            ...
            ImageView imageView = (ImageView) childAt.getChildAt(1);
