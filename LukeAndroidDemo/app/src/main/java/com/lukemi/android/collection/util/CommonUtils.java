...
//                .asBitmap()//（可选）只允许加载静态图片，不需要Glide去帮我们自动进行图片格式的判断了（gif图片就只能先是一帧）
                .animate(com.lukemi.android.collection.R.anim.alpha)//设置动画
//                        .asGif()//（可选）只允许加载动态图片，和上面的只能选一个
                .placeholder(com.lukemi.android.collection.R.drawable.ic_launcher)//（可选）设置默认占位图
                .error(com.lukemi.android.collection.R.drawable.ic_launcher)//（可选）设置异常占位图
...
//                .setIndicatorsEnabled(true)//显示指示器
                .load(url)//（可选）传入图片网址
                .placeholder(com.lukemi.android.collection.R.drawable.ic_launcher)//（可选）设置默认占位图
                .error(com.lukemi.android.collection.R.drawable.ic_launcher)//（可选）设置出错占位图
//                .noFade()//（可选）不设置渐入的效果
