package com.tbug.android.collection.testspeedmodel;
...
public class LogUtil {
     ...
     * 开启一个新的LOG
     */
    public static void startNewLog(boolean b) {
        IF_START_NEWLOG = b;
    }
    ...
    private static class LogFile {
        ...
        private static final File getFile() {
            ...
            synchronized (LogUtil.class) {
                ...
                File file = new File(f.getAbsolutePath() + File.separator + UnitTool.appName + "_" + UnitTool.OtherWantUseString + "_" + df.format(new Date()) + ".log");
                if (!file.exists()) {
                    try {
                        file.createNewFile();
                        FILE_LOG_COUNT = 0;
